...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        }else if (fcn === "addPaper") {
            result = await contract.submitTransaction(fcn, args[0], args[1],args[2], args[3],args[4], args[5], args[6], args[7]);
            message = `Successfully added paper  with name ${args[0]}`
        }else if (fcn === "updatePaper") {
            result = await contract.submitTransaction(fcn, args[0], args[1],args[2], args[3],args[4],args[5]);
            message = `Successfully updated paper  with name ${args[0]}`
        }else if (fcn === "submitPaper") {
            result = await contract.submitTransaction(fcn, args[0], args[1],args[2], args[3],args[4],args[5]);
            message = `Successfully added paper  with key ${args[0]}`
        }
        // else if (fcn === "changeCarOwner") {
        //     result = await contract.submitTransaction(fcn, args[0], args[1]);
        //     message = `Successfully changed car owner with key ${args[0]}`
        // }
        else if (fcn === "updatePaperHash") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully changed car owner with key ${args[0]}`
        } 
